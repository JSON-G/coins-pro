 ...
 * 
 * @param {JSONobject} - JSON (already parsed) object which contains the client information such as APIKey, secret, UserId, and AccountId
 * @returns {Promise}
 */
 ...
 * send buy order
 * 
 * @param {number} InstrumentId - for example InstrumentId 6 is BTCPHP which means buy X Quantity of BTC using PHP wallet
 * @param {number} Quantity - amount to buy. for example, as of this writing: 10 pesos = 0.00001740 BTC. Therefore, if you wanted to buy 10 pesos worth of BTC, pass 0.00001740 as Quantity
 */
function buy({ InstrumentId = 6, Quantity = 0.00001740 } = {}) {
    sendMessage({
        message_type: 0,
 ...
 * send sell order
 * 
 * @param {number} InstrumentId - for example InstrumentId 6 is BTCPHP which means sell X Quantity of BTC for Y PHP. value is volatile
 * @param {number} Quantity - amount to sell
 */
function sell({ InstrumentId = 6, Quantity = 0.00001730 } = {}) {
    sendMessage({
        message_type: 0,
        function_name: "SendOrder",
        payload: JSON.stringify({
