...
function emitWebsocketMessage({ message_type, sequence_number, function_name, payload }) {
    ...
    } else if (function_name === "SubscribeLevel2" || function_name === "Level2UpdateEvent") {
    ...
    } else if (function_name === "SubscribeTrades" || function_name === "TradeDataUpdateEvent") {
...
function subscribeToData({ message_type, function_name, payload, callback }) {
    ...
    // emitter event name is concatenated with function_name for subscriptions
    ...
    let event_name = `${function_name}-instrument-${InstrumentId}`;
...
function unsubscribeLevel1(InstrumentId, callback) {
    sendMessage({
        message_type: 0,
...
function subscribeLevel2({ InstrumentId, Depth }, callback) {
    subscribeToData({
        message_type: 0,
...
function unsubscribeLevel2(InstrumentId, callback) {
    sendMessage({
        message_type: 0,
 ...
 * @param {Function} callback
 */
function subscribeTrades({ InstrumentId, IncludeLastCount }, callback) {
    subscribeToData({
        message_type: 0,
...
function unsubscribeTrades(InstrumentId, callback) {
    sendMessage({
        message_type: 0,
